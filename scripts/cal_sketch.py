...
def cal_sketch(pointing_ang=50,     # QUBIC nominal pointing elevation angle
               ...
               ylength=8,           # y extent of the drawing
               transparency=False,
               figsize=(16,10)):
    ...
    x=window_pos[0]
    y=window_pos[1]
    plt.plot(x,y,color='red',linestyle='none',marker='D')
    txt='   QUBIC position at (%.2fm, %.2fm)' % (D,Hqubic)
    plt.text(x,y,txt,color='red',fontsize=fontsize,ha='left',va='center')

    # cal source position marker
    x=cal_pos[0]
    y=cal_pos[1]
    plt.plot(x,y,color='green',linestyle='none',marker='D')
    txt='cal source at height %.2fm  ' % cal_pos[1]
    plt.text(x,y,txt,color='green',fontsize=fontsize,ha='right',va='bottom')
    ...
    plt.text(x2,y2,txt,color='black',fontsize=fontsize,ha='left',va='center')
    ...
    #plt.plot([x[1],x[1]],[y[1],0],color='black',linestyle='dashed')

    txt='mirror horizontal extension = %.2fm' % abs(2*mirror_dx)
    plt.text(x[0],y[0],txt,ha='right',va='bottom',fontsize=fontsize,color='black')
    
    if mirror_ang > 90:
        txt='  mirror angle = %.2f$^\circ$' % (mirror_ang-90)
    else:
        txt='  mirror angle = %.2f$^\circ$' % mirror_ang        
    plt.text(x[0],y[0],txt,ha='left',va='center',fontsize=fontsize,color='black')
    ...
    x=mirror_pos[0]
    y=mirror_pos[1]
    txt=' flat mirror at (%.2fm, %.2fm)' % (abs(mirror_pos[0]),mirror_pos[1])
    plt.text(x,y,txt,color='black',ha='right',va='bottom',fontsize=fontsize)
    ...
    x=[1-xlength,1]
    y=[0,0]
    plt.plot(x,y,color='black',linewidth=0.5,linestyle='dotted')
    ...
    #plt.plot(top_centre[0],top_centre[1],marker='X',color='black')
    print win_offset
    print top_centre
    print bot_centre
    ...
    plt.text(0.5,0.1,txt,va='top',ha='center',fontsize=fontsize,transform=ax.transAxes)

    plt.title('QUBIC Calibration Source layout')
    
    pngname='/home/work/qubic/pix/QUBIC_calibration_source_layout_sketch.png'
    plt.savefig(pngname,format='png',dpi=100,bbox_inches='tight',transparent=True)
    return
